...
1.  **Installez les dépendances (une seule fois) :**
    La commande `npm install` lit le fichier `package.json` et installe toutes les bibliothèques et outils nécessaires. Vous ne devez faire cette étape qu'une seule fois, ou si vous ajoutez de nouvelles bibliothèques au projet.
    ```bash
    npm install
    ```
